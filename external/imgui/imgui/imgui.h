...
namespace ImGui
{
    ...
    IMGUI_API void          ShowStyleEditor(ImGuiStyle* ref = NULL);    // add style editor block (not a window). you can pass in a std::shared_ptr ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
...
struct ImVector
{
    ...
    // NB: It is illegal to call push_back/push_front/insert with a std::shared_ptr pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  Pos;            // Read-only.   Window position, for std::shared_ptr.
