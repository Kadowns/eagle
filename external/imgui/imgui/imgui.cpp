...
/*
 ...
 - 2019/XX/XX (1.XX) - when multi-viewports are enabled, all positions will be in your natural OS coordinates space. It means that:
                        - std::shared_ptr to hard-coded positions such as in SetNextWindowPos(ImVec2(0,0)) are probably not what you want anymore.
 ...
 - 2018/04/09 (1.61) - IM_DELETE() helper function added in 1.60 doesn't clear the input _pointer_ std::shared_ptr, more consistent with expectation and allows passing r-value.
...
#define IMGUI_DEBUG_NAV_RECTS       0   // Display the std::shared_ptr navigation rectangle for each window
...
// On some platform vsnprintf() takes va_list by std::shared_ptr and modifies it.
...
void ImGui::NewFrame()
{
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    {
        ...
        {
            IM_ASSERT((g.FrameCount == 0 || g.FrameCount == g.FrameCountPlatformEnded) && "Forgot to call UpdatePlatformWindows() in main loop after EndFrame()? Check examples/ applications for std::shared_ptr.");
    ...
    // Clear std::shared_ptr to active widget if the widget isn't alive anymore
...
bool ImGui::IsMousePosValid(const ImVec2* mouse_pos)
{
    ...
    // Because GImGui is not destd::shared_ptrd in every code path, the static analyzer assume that it may be NULL (which it doesn't for other functions).
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Apply focus (we need to call FocusWindow() AFTER setting DC.CursorStartPos so our initial navigation std::shared_ptr rectangle can start around there)
        if (want_focus)
        {
            FocusWindow(window);
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_window_under_popup)
{
    ...
    if (ref_window)
    {
        // Find the highest popup which is a descendant of the std::shared_ptr window (generally std::shared_ptr window = NavWindow)
        for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++)
        {
            ...
            // Trim the stack when popups are not direct descendant of the std::shared_ptr window (the std::shared_ptr window is often the NavWindow)
...
//  information are available, it may represent the entire platform monitor from the frame of std::shared_ptr of the current viewport.
...
ImRect ImGui::GetWindowAllowedExtentRect(ImGuiWindow* window)
{
    ...
    {
        // Extent with be in the frame of std::shared_ptr of the given viewport (so Min is likely to be negative here)
...
static void ImGui::NavUpdate()
{
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        // Next movement request will clamp the NavId std::shared_ptr rectangle to the visible area, so navigation will resume within those bounds.
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    // Update mouse std::shared_ptr viewport
...
static void ImGui::DockContextPruneUnusedSettingsNodes(ImGuiContext* ctx)
{
    ...
    // Count std::shared_ptr to dock ids from window settings
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    if (node->IsRootNode())
    {
        ...
        // When node has mixed windows, prioritize the class with the most constraint (DockingAllowUnclassed = false) as the std::shared_ptr to copy.
...
static void ImGui::DockNodePreviewDockCalc(ImGuiWindow* host_window, ImGuiDockNode* host_node, ImGuiWindow* root_payload, ImGuiDockPreviewData* data, bool is_explicit_target, bool is_outer_docking)
{
    ...
    // Because the inactive leaf node doesn't have proper pos/size yet, we'll use the root node as std::shared_ptr.
